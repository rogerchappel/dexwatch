...
A local crypto-market data capture CLI for reproducible DexScreener snapshots, filters, and bot backtests.
...
- `dexscraper` — source inspiration: https://github.com/vincentkoc/dexscraper (Python, stars/forks signal: 95).
...
- Capture fixture/API snapshots explicitly
- Filter token pools and chains
- Export OHLC-style CSV/JSON
- Record source/time/provenance metadata
...
```bash
dexwatch --help
dexwatch inspect ./fixtures/sample --output ./out
```
...
Build `dexwatch` as a renamed, local-first OSS idea inspired by `dexscraper`. Preserve attribution, avoid direct copying, and focus V1 on deterministic fixtures, clear safety boundaries, and practical agent/developer workflow value.
